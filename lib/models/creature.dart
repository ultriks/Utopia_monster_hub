...
class Creature {
  ...
  factory Creature.fromMap(Map<String, dynamic> map) {
    return Creature(
      ...
      bodyType: CreatureBodyType.values.firstWhere(
        (e) => e.name == map['body_type'],
        orElse: () => CreatureBodyType.humanoid,
      ),
      kits: List<Map<String, dynamic>>.from(
        jsonDecode(map['kits_json'] ?? '[]'),
      ),
      ...
      customStats: Map<String, int>.from(
        jsonDecode(map['custom_stats_json'] ?? '{}'),
      ),
      actions: List<CreatureAction>.from(
        (jsonDecode(map['actions_json'] ?? '[]') as List).map(
          (x) => CreatureAction.fromMap(x),
        ),
      ),
  ...
  CreatureBody get _baseBody => creatureBodies.firstWhere(
    (b) => b.type == bodyType,
    orElse: () => creatureBodies.first,
  );
  ...
  int getBmsStat(String statName) {
    ...
    for (var c in _activeClasses) {
      val += c.bmsStatsBonus[statName] ?? 0;
    }
    return val;
  }

  int getTravel(String name) {
    int val = 0;
    final currentSpeed = getSubtrait('speed');
    switch (name) {
      case 'land':
        val = currentSpeed;
        break;
      case 'water':
        val = (currentSpeed / 2).ceil();
        break;
      case 'air':
        val = 0;
        break;
    ...
    for (var k in _activeKits) {
      val += k.travelBonus[name] ?? 0;
    }
    for (var c in _activeClasses) {
      val += c.travelBonus[name] ?? 0;
    }
    return val;
  }
      ...
      if (item is Weapon) {
        ...
        list.add(
          CreatureAction(
            name: item.name,
            description:
                "Attack with ${item.name} (TA: ${item.TA}, Damage: $dmg ${item.damageType.name}). Range: $rng.",
          ),
        );
      }
    }
